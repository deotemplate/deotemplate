...
//        $this->module_path_resource = $this->module_path.'views/';
		...
		if (isset($files_content->onepagecheckout) && $files_content->onepagecheckout)
		{
			foreach ($files_content->onepagecheckout->children() as $onepagecheckout) {
				if ($override) {
					...
					$obj_model->fullwidth = $onepagecheckout->fullwidth->__toString();
					$obj_model->active = 0;
	...
	public function postProcess()
	{
		...
		if (Tools::isSubmit('generateall')) {
			...
			foreach ($result as $value) {
				...
				$this->saveTplFile($value['plist_key'], $value['params'], $value['class_checkout'], $data_form['class'], $value['fullwidth']);
			}
	...
	//save file
	public function saveTplFile($plist_key, $params = '', $class_checkout = null, $main_class = null, $fullwidth = null)
	{
