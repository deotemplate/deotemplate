...
class AdminDeoReviewManagerController extends ModuleAdminController
{
    public function __construct()
    {
        parent::__construct();
        $this->bootstrap = true;
        $this->table = 'deofeature_product_review_criterion';
        $this->identifier = 'id_deofeature_product_review_criterion';
        $this->className = 'DeoProductReviewCriterion';
        $this->lang = true;
        ...
        $this->_defaultOrderBy = 'id_deofeature_product_review_criterion';
        $this->_select .= ' CASE
            WHEN `id_deofeature_product_review_criterion_type` = 1 THEN "'.$this->module->l('Valid for the entire catalog').'"
            WHEN `id_deofeature_product_review_criterion_type` = 2 THEN "'.$this->module->l('Restricted to some categories').'"
            WHEN `id_deofeature_product_review_criterion_type` = 3 THEN "'.$this->module->l('Restricted to some products').'"
            END as `type_name` ';
    }
    ...
    public function renderForm()
    {
        ...
        $types = DeoProductReviewCriterion::getTypes();
        $query = array();
        ...
        if (Tools::getValue('id_deofeature_product_review_criterion')) {
            $id_criterion = Tools::getValue('id_deofeature_product_review_criterion');
        } else {
            $id_criterion = 0;
        }
        
        $criterion = new DeoProductReviewCriterion((int) $id_criterion);
        ...
        $this->fields_form = array(
            'legend' => array(
                'title' => $this->l('Review Criterion'),
                'icon' => 'icon-cogs',
            ),
            'input' => array(
                array(
                    'type' => 'hidden',
                    'name' => 'id_deofeature_product_review_criterion',
                ),
                array(
                    'type' => 'text',
                    'lang' => true,
                ...
                array(
                    'type' => 'select',
                    'name' => 'id_deofeature_product_review_criterion_type',
    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('submitEditCriterion') || Tools::isSubmit('submitEditCriterionAndStay')) {
            ...
            if ((int) Tools::getValue('id_deofeature_product_review_criterion')) {
                $mess_id = '4';
            } else {
                $mess_id = '3';
            }
            
            $criterion = new DeoProductReviewCriterion((int) Tools::getValue('id_deofeature_product_review_criterion'));
            $criterion->id_deofeature_product_review_criterion_type = Tools::getValue('id_deofeature_product_review_criterion_type');
            ...
            if ($criterion->id_deofeature_product_review_criterion_type == 2) {
            ...
            } elseif ($criterion->id_deofeature_product_review_criterion_type == 3) {
        ...
        } elseif (Tools::isSubmit('deletedeofeature_product_review') && Tools::getValue('id_deofeature_product_review')) {
            $id_deofeature_product_review = (int) Tools::getValue('id_deofeature_product_review');
            $review = new DeoProductReview($id_deofeature_product_review);
            $review->delete();
            // var_dump($review->delete());die();
            $this->redirect_after = self::$currentIndex.'&conf=1&token='.$this->token;
        } elseif (Tools::isSubmit('approveReview') && Tools::getValue('id_deofeature_product_review')) {
            $id_deofeature_product_review = (int) Tools::getValue('id_deofeature_product_review');
            $review = new DeoProductReview($id_deofeature_product_review);
            $review->validate();
            $this->redirect_after = self::$currentIndex.'&conf=4&token='.$this->token;
        } elseif (Tools::isSubmit('noabuseReview') && Tools::getValue('id_deofeature_product_review')) {
            $id_deofeature_product_review = (int) Tools::getValue('id_deofeature_product_review');
            DeoProductReview::deleteReports($id_deofeature_product_review);
            $this->redirect_after = self::$currentIndex.'&conf=4&token='.$this->token;
        } else {
            parent::postProcess(true);
        }

        // $this->module->_clearcache('feature/deo_list_product_review.tpl');
    }
    ...
    public function initCategoriesAssociation($id_root = null, $id_criterion = 0)
    {
        ...
        if ($id_criterion == 0) {
            ...
            $pdc_object = new DeoProductReviewCriterion($id_criterion);
    ...
    // render list reviews have not approved and reported
    public function renderModerateLists()
    {
        ...
        if ((int) DeoHelper::getConfig('PRODUCT_REVIEWS_MODERATE')) {
            ...
            if (count($reviews) > 0) {
                ...
                $helper->identifier = 'id_deofeature_product_review';
                $helper->title = $this->l('Reviews waiting for approval');
                $helper->table = 'deofeature_product_review';
        ...
        $reviews = DeoProductReview::getReportedReviews();
    ...
    // render list reviews have approved
    public function renderReviewsList()
    {
       
        $reviews = DeoProductReview::getByValidate(1, false);
        $moderate = (int) DeoHelper::getConfig('PRODUCT_REVIEWS_MODERATE');
        if (empty($moderate)) {
            $reviews = array_merge($reviews, DeoProductReview::getByValidate(0, false));
        }
    ...
    public function displayApproveLink($token, $id, $name = null)
    {
        ...
        $template->assign(array(
            'href' => $this->context->link->getAdminLink('AdminDeoReviewManager').'&approveReview&id_deofeature_product_review='.$id,
    ...
    public function displayNoabuseLink($token, $id, $name = null)
    {
        ...
        $template->assign(array(
            'href' => $this->context->link->getAdminLink('AdminDeoReviewManager').'&noabuseReview&id_deofeature_product_review='.$id,
    ...
    {
        return array(
            'id_deofeature_product_review' => array(
                'title' => $this->l('ID'),
                'type' => 'text',
            ),
            'title' => array(
    ...
    public function initProcess()
    {
        ...
        if (count($this->errors) <= 0) {
            if (!$this->access('delete')){
                if (Tools::isSubmit('deletedeofeature_product_review')) {
