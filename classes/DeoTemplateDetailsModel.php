...
class DeoTemplateDetailsModel extends ObjectModel
{
    public $name;
    public $params;
    public $fullwidth;
    ...
    public static $definition = array(
        'table' => 'deotemplate_details',
        'primary' => 'id_deotemplate_details',
        'multilang' => false,
        'multishop' => true,
        'fields' => array(
    ...
    public function getAllProductProfileByShop()
    {
        ...
        $sql = 'SELECT p.*, ps.*
                 FROM '._DB_PREFIX_.'deotemplate_details p
                 INNER JOIN '._DB_PREFIX_.'deotemplate_details_shop ps ON (ps.id_deotemplate_details = p.id_deotemplate_details)'
                .$where;
    ...
    public function add($autodate = true, $null_values = false)
    {
        ...
        $res &= Db::getInstance()->execute('
                INSERT INTO `'._DB_PREFIX_.'deotemplate_details_shop` (`id_shop`, `id_deotemplate_details`)
                VALUES('.(int)$id_shop.', '.(int)$this->id.')');
        if (Db::getInstance()->getValue('SELECT COUNT(p.`id_deotemplate_details`) AS total FROM `'
                        ._DB_PREFIX_.'deotemplate_details` p INNER JOIN `'
                        ._DB_PREFIX_.'deotemplate_details_shop` ps ON(p.id_deotemplate_details = ps.id_deotemplate_details) WHERE id_shop='
                        .(int)$id_shop) <= 1) {
            $this->deActiveAll();
        } else if ($this->active) {
            $this->deActiveAll();
    ...
    {
        $id_shop = DeoHelper::getIDShop();
        $sql = 'UPDATE '._DB_PREFIX_.'deotemplate_details_shop SET active=0 where id_shop='.(int)$id_shop;
        Db::getInstance()->execute($sql);
        $where = ' WHERE ps.id_shop='.(int)$id_shop." AND ps.id_deotemplate_details = '".(int)$this->id."'";
        Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'deotemplate_details_shop` ps set ps.active = 1 '.$where);
    }
    ...
    public static function getActive()
    {
        ...
        $sql = 'SELECT * FROM '._DB_PREFIX_.'deotemplate_details p
                INNER JOIN '._DB_PREFIX_.'deotemplate_details_shop ps on(p.id_deotemplate_details = ps.id_deotemplate_details) WHERE '
                .pSQL($where);
