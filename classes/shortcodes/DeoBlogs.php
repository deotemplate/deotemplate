...
class DeoBlogs extends DeoShortCodeBase
{
    public $name = 'DeoBlogs';
    ...
    public function getInfo()
    {
        return array(
            'label' => 'Blogs', 
            'position' => 3, 
            'desc' => $this->l('You can display blog'),
            'image' => 'blog.png',
            'tag' => 'content slider',
    ...
    public function getConfigList()
    {
        if (!(int) DeoHelper::getConfig('ENABLE_BLOG')) {
            return array(
                array(
                    'type' => 'html',
                    'name' => 'default_html',
                    'html_content' => '<div class="alert alert-warning">'.$this->l('Please enable blog to use this widget.').'</div>'
                )
            );
        }
        ...
        //$list = array();
        $controller = 'AdminDeoBlogDashboard';
        ...
        if (Tools::getIsset('chk_cat')) {
            $value_by_categories = Tools::getValue('chk_cat');
            $arr_key = explode(',', $value_by_categories);
            
            $randkey_cat = '';
            foreach ($arr_key as $val) {
                $randkey_cat .= ($randkey_cat == '') ? "'".pSQL($val)."'" : ",'".pSQL($val)."'";
            }
            $sql = 'SELECT * FROM `'._DB_PREFIX_.'deoblog_category`';
            $sql .= ' WHERE randkey IN ('.$randkey_cat.')';
        
            $arr_cat = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);

            foreach ($arr_cat as $cat) {
                $selected_categories[] = $cat['id_deoblog_category'];
            }
        }
        
        $data_category = $category->getTreeForPageBuilder($selected_categories);

        $configuration = new DeoBlogConfiguration();
        $image_size = array(
        ...
        $inputs_owlCarousel = array(
            ...
            array(
                'type' => 'text',
                'name' => 'items',
                'label' => $this->l('Desktop Large ( width screen ≥ 1500 )'),
        ...
        $inputs_content = array(
            ...
            array(
                ...
                'name' => 'default_html',
                'html_content' => '<div class="sperator"></div>',
                'form_group_class' => 'value_by_blog_id',
            ),
            array(
                'type' => 'text',
                'name' => 'blog_id',
                'label' => $this->l('Blog ID'),
                'desc' => $this->l('Show blog follow blog id. Example: 1 or 1,2,3,4 '),
                'default' => '',
                'form_group_class' => 'value_by_blog_id',
            ),
            array(
                ...
                'name' => 'default_html',
                'html_content' => '<div class="sperator"></div>',
                'form_group_class' => 'value_by_author',
            ),
            array(
                'type' => 'text',
                'name' => 'author',
                'label' => $this->l('Author'),
                'desc' => $this->l('Show blog follow author name.'),
                'default' => '',
                'form_group_class' => 'value_by_author',
            ),
            array(
                ...
                'name' => 'default_html',
                'html_content' => '<div class="sperator"></div>',
                'form_group_class' => 'value_by_tag',
            ),
            array(
                'type' => 'text',
                'name' => 'tag',
                'label' => $this->l('Tag'),
                'desc' => $this->l('Show blog follow tag name.'),
                'default' => '',
                'form_group_class' => 'value_by_tag',
            ),
            array(
            ...
            array(
                ...
                'name' => 'order_by',
                'options' => array(
                    'query' => DeoSetting::getOrderByBlog(),
                    'id' => 'id',
                    'name' => 'name'
                ),
            ...
            array(
                'type' => 'html',
                'name' => 'default_html',
                'html_content' => '<div class="alert alert-info">'.$this->l('Step 3: Configuration for blog item').'</div>',
            ),
            array(
                'type' => 'select',
                ...
                'default' => '',
                'desc' => 'Choose images type for image blog.'
            ),
            array(
                'type' => 'switch',
            ...
            array(
                'type' => 'switch',
                'label' => $this->l('Show Link View All Blogs'),
                'name' => 'show_view_all',
                'values' => DeoSetting::returnYesNo(),
                'default' => 1,
            ),     
    ...
    public function prepareFontContent($assign, $module = null)
    {
        if ((int) DeoHelper::getConfig('ENABLE_BLOG')) {
            if (!DeoHelper::getLazyload()) {
                ...
                if (isset($assign['formAtts']['image_size']) && $assign['formAtts']['image_size'] && !empty($image_size)){
                    $assign['formAtts']['rate_image'] = $image_size[$assign['formAtts']['image_size']]->rate.'%';
                }
            }
            ...
            //         $assign['formAtts']['lib_error'] = 'Can not show Blog. Please enable Owl Carousel library in Configuration.';
            //         return $assign;
            //     }
            // }
            ...
            $assign['formAtts']['helper'] = DeoBlogHelper::getInstance();
            $assign['products'] = $this->getBlogsFont($assign['formAtts'], $helper);
            ...
            $assign['formAtts']['lib_has_error'] = true;
            $assign['formAtts']['lib_error'] = 'Please enable blog to use this widget.';
        }
        return $assign;
    }
    ...
    /**
     * Get lastest blog for DeoTemplate module
     * @param type $params
     * @return type
     */
    public function getBlogsFont($params, $helper)
    {   
        $context = Context::getContext();

        $order_by = isset($params['order_by']) ? $params['order_by'] : 'id_deoblog';
        $order_way = isset($params['order_way']) ? $params['order_way'] : 'DESC';

        $condition = array();
        if (isset($params['value_by_categories']) && isset($params['chk_cat'])){
            // GET ID_BLOG VIA randkey - BEGIN
            $randkey_cat = '';
            $arr_key = explode(',', $params['chk_cat']);
            foreach ($arr_key as $val) {
                $randkey_cat .= ($randkey_cat == '') ? "'".pSQL($val)."'" : ",'".pSQL($val)."'";
            }
            $condition['chk_cat'] = $randkey_cat;
        }

        if (isset($params['value_by_blog_id']) && isset($params['blog_id']) && $params['blog_id'] != ''){
            $condition['blog_id'] = $params['blog_id'];
        }

        if (isset($params['value_by_author']) && isset($params['author']) && $params['author'] != ''){
            $condition['author'] = $params['author'];
        }

        if (isset($params['value_by_tag']) && isset($params['tag']) && $params['tag'] != ''){
            $condition['tag'] = $params['tag'];
        }

        $limit = (int)$params['nb_blogs'];
        $blogs = $this->getListBlogsPageBuilder(null, $context->language->id, $limit, $order_by, $order_way, $condition, true);
        ...
        foreach ($blogs as $key => &$blog) {
            $image_size = (isset($params['image_size']) && $params['image_size']) ? $params['image_size'] : false;
            $blog = $helper->formatBlog($blog, $image_size, $comment);
    ...
    /**
     * This function is build for module DeoTemplate, most logic query Sqlis clone from function getListBlog.
     ...
     */
    public static function getListBlogsPageBuilder($id_category, $id_lang, $nb_blog, $order_by, $order_way, $condition, $is_active, $id_shop = null)
    {
        # module validation
        ...
        if ($id_category) {
            ...
            $where .= ' AND c.id_deoblog_category IN('.$id_category.')';
        }
        if ($id_shop) {
            ...
            switch ($key) {
                ...
                case 'tag':
                    ...
                    if (!empty($tmp) && count($tmp) > 1) {
                        ...
                        $where .= ' AND l.meta_keywords LIKE "%'.pSQL($value).'%"';
                    }
                    break;
                case 'samecat':
                    $where .= ' AND c.id_deoblog!='.(int)$value;
                case 'chk_cat':
                    if ($value_by_categories) {
                        $where .= ' AND bc.randkey IN ('.$value_by_categories.')';
                    }
                case 'blog_id':
                    if ($value) {
                        $temp = explode(',', $value);
                        ...
                        $blog_id = implode(',', array_map('intval', $temp));
                        $where .= ' AND c.id_deoblog '.(Tools::strpos($blog_id, ',') === false ? '= '.(int)$blog_id : 'IN ('.pSQL($blog_id).')');
                    }
                    break;
            }
        }
        ...
        $query = '
            ...
            FROM  '._DB_PREFIX_.'deoblog c
            LEFT JOIN '._DB_PREFIX_.'deoblog_lang l ON (c.id_deoblog = l.id_deoblog) and  l.id_lang='.(int)$id_lang
                .' LEFT JOIN '._DB_PREFIX_.'deoblog_shop s ON  (c.id_deoblog = s.id_deoblog) '
                .' LEFT JOIN '._DB_PREFIX_.'deoblog_category bc ON  bc.id_deoblog_category = c.id_deoblog_category '
