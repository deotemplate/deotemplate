...
class DeoTemplateProfilesModel extends ObjectModel
{
    public $name;
    public $friendly_url;
    public $meta_title;
    ...
    public static $definition = array(
        'table' => 'deotemplate_profiles',
        'primary' => 'id_deotemplate_profiles',
        'multilang' => true,
        'multishop' => true,
        'fields' => array(
            ...
            'mobile' => array('type' => self::TYPE_INT, 'validate' => 'isUnsignedId'),
    ...
    public function deActiveAll($page)
    {
        ...
        //$where = ' WHERE ps.id_shop='.$id_shop." AND ps.id_deotemplate_profiles != '".(int)$this->id."'";
        $where = ' WHERE ps.id_shop='.(int)$id_shop;
        Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'deotemplate_profiles_shop` ps set ps.active = 0 '.$where);
        $where = ' WHERE ps.id_shop='.$id_shop." AND ps.id_deotemplate_profiles = '".(int)$this->id."'";
        Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'deotemplate_profiles_shop` ps set ps.active = 1 '.$where);
    }
    ...
    public function getAllProfileByShop()
    {
        try {
            ...
            $sql = 'SELECT p.*, ps.*,pl.friendly_url
                        ...
                        INNER JOIN '._DB_PREFIX_.'deotemplate_profiles_lang pl ON (p.id_deotemplate_profiles = pl.id_deotemplate_profiles)
    ...
    public function add($autodate = true, $null_values = false)
    {
        ...
        $res &= Db::getInstance()->execute('
                INSERT INTO `'._DB_PREFIX_.'deotemplate_profiles_shop` (`id_shop`, `id_deotemplate_profiles`)
                VALUES('.(int)$id_shop.', '.(int)$this->id.')');
        if (Db::getInstance()->getValue('SELECT COUNT(p.`id_deotemplate_profiles`) AS total FROM `'._DB_PREFIX_.'deotemplate_profiles` p
                INNER JOIN `'._DB_PREFIX_.'deotemplate_profiles_shop` ps ON(p.id_deotemplate_profiles = ps.id_deotemplate_profiles)
    ...
    public function getProfilesInPage($id = 0)
    {
        ...
        if ($id) {
            $where .= ' AND p.id_deotemplate_profiles !='.(int)$id;
        }
        $inner_join = 'INNER JOIN `'._DB_PREFIX_.'deotemplate_profiles_shop` ps ON (ps.id_deotemplate_profiles = p.id_deotemplate_profiles)';
        $sql = 'SELECT p.* from `'._DB_PREFIX_.'deotemplate_profiles` p '.$inner_join.$where;
        return Db::getInstance()->executes($sql);
    }
    ...
    public static function getActiveProfile($page)
    {
        ...
        $result = null;
        $is_use_co = (int) DeoHelper::getConfig('SAVE_COOKIE_PROFILE');
        $context = Context::getContext();
        $id_shop = $context->shop->id;
        $id_profile = DeoTemplateProfilesModel::getIdProfileFromRewrite();
            ...
            foreach ($list_profiles as $profile) {
                ...
                foreach ($aray_group_box as $group_box) {
                    if (isset($current_group_id) && $current_group_id == $group_box) {
                        $profile_current = $profile['id_deotemplate_profiles'];
                    }
                }
            }
        }
        ...
        //check cookie
        if ($is_use_co) {
            if (!$id_profile && $context->cookie->deo_profile) {
                $id_profile = $context->cookie->deo_profile;
            } else {
                $context->cookie->deo_profile = $id_profile;
            }
        }

        $id_lang = $context->language->id;
        $cache_id = 'DeoTemplateProfilesModel::getActiveProfile_'.md5((int)$id_profile.(int)$id_lang.(int)$id_shop);
        if (!Cache::isStored($cache_id)) {
            if ($id_profile) {
                $where = ' WHERE ps.id_shop='.(int)$id_shop.' AND p.id_deotemplate_profiles='.(int)$id_profile;
            } else {
                $where = ' WHERE ps.id_shop='.(int)$id_shop.' AND ps.active=1 ';
            }
            $inner_join = ' INNER JOIN `'._DB_PREFIX_.'deotemplate_profiles_lang` pl ON (pl.id_deotemplate_profiles = p.id_deotemplate_profiles) AND id_lang='.(int)$id_lang;
            $inner_join .= ' INNER JOIN `'._DB_PREFIX_.'deotemplate_profiles_shop` ps ON (ps.id_deotemplate_profiles = p.id_deotemplate_profiles)';
            $sql = 'SELECT p.*, pl.* from `'._DB_PREFIX_.'deotemplate_profiles` p '.$inner_join.$where;
            $result = Db::getInstance()->getRow($sql);
            Cache::store($cache_id, $result);
        } else {
            $result = Cache::retrieve($cache_id);
        }
        foreach (array('mobile', 'header', 'content', 'footer', 'product') as $val) {
            $pos_key = 'deo_'.$val;
    ...
    {
        $sql = 'SELECT * FROM `'._DB_PREFIX_.'deotemplate_profiles` WHERE id_deotemplate_profiles='.(int)$id;
    ...
    public function duplicateProfile($id, $name, $profile_key, $id_shop)
    {
        ...
        if ($object_duplicated) {
            $sql = 'INSERT INTO `'._DB_PREFIX_.'deotemplate_profiles`(name, profile_key, page) VALUES("'
                    .pSQL($name).pSQL($object_duplicated['name']).'", "'.pSQL($profile_key).'", "index")';
            Db::getInstance()->execute($sql);
            $new_id = Db::getInstance()->Insert_ID();
            $sql = 'INSERT INTO `'._DB_PREFIX_.'deotemplate_profiles_shop`(id_deotemplate_profiles, id_shop, active) VALUES('
    ...
    public static function deleteById($id)
    {
        ...
        $object_duplicated = Db::getInstance()->getRow($sql);
        if ($object_duplicated) {
            $sql = 'DELETE FROM `'._DB_PREFIX_.'deotemplate_profiles` WHERE id_deotemplate_profiles='.(int)$id;
            Db::getInstance()->execute($sql);
            $sql = 'DELETE FROM `'._DB_PREFIX_.'deotemplate_profiles_lang` WHERE id_deotemplate_profiles='.(int)$id;
            Db::getInstance()->execute($sql);
            $sql = 'DELETE FROM `'._DB_PREFIX_.'deotemplate_profiles_shop` WHERE id_deotemplate_profiles='.(int)$id;
            Db::getInstance()->execute($sql);
            return $object_duplicated;
        }
        return array();
    ...
    public function getPositionsForProfile($id_positions)
    {
        if ($id_positions) {
            ...
            $sql = 'SELECT * FROM `'._DB_PREFIX_.'deotemplate_positions` WHERE id_deotemplate_positions IN('.pSQL($id_positions).')';
            return Db::getInstance()->executes($sql);
        }
        return array();
    }
        ...
        if ($result) {
            if (isset($this->def['multishop']) && $this->def['multishop'] == true) {
                ...
                if (isset($params['import_sample']) && $params['import_sample'] == true) {
                    $position = new DeoTemplatePositionsModel((int)$this->mobile);
                    $position->delete();
                    $position = new DeoTemplatePositionsModel((int)$this->header);
                    $position->delete();
                    $position = new DeoTemplatePositionsModel((int)$this->content);
                    $position->delete();
                    $position = new DeoTemplatePositionsModel((int)$this->footer);
                    $position->delete();
                    $position = new DeoTemplatePositionsModel((int)$this->product);
                    $position->delete();
                }
            }
        }
    ...
    {
        static $id_deotemplate_profiles = null;
        
        if ($id_deotemplate_profiles === null) {
            $context = Context::getContext();
            $id_shop = (int)$context->shop->id;
            $id_lang = (int)$context->language->id;
                
            # ACCESS BY id_deotemplate_profiles
            $id_temp = Tools::getIsset('id_deotemplate_profiles') ? Tools::getValue('id_deotemplate_profiles') : false;
            if ($id_temp !== false) {
                $id_deotemplate_profiles = $id_temp;
                return $id_deotemplate_profiles;
            }

            # ACCESS BY friendly_url
            $linkRewrive = explode('/', $_SERVER['REQUEST_URI']);
            $linkRewrive = rtrim(end($linkRewrive), '.html');
            if (Tools::strpos($linkRewrive, '?')) {
                // REMOVE ? FROM URL
            ...
            if (!empty($linkRewrive)) {
                $sql = 'SELECT p.`id_deotemplate_profiles` FROM `'._DB_PREFIX_.'deotemplate_profiles` p';
                $sql .= ' INNER JOIN `'._DB_PREFIX_.'deotemplate_profiles_lang` pl ON(pl.id_deotemplate_profiles = p.id_deotemplate_profiles AND pl.friendly_url=\''.pSQL($linkRewrive).'\' AND id_lang='.(int)$id_lang.' ) ';
                $sql .= ' INNER JOIN `'._DB_PREFIX_.'deotemplate_profiles_shop` ps ON(ps.id_deotemplate_profiles = p.id_deotemplate_profiles AND ps.id_shop ='.(int)$id_shop.')';

                $id_deotemplate_profiles = Db::getInstance()->getValue($sql);
                return $id_deotemplate_profiles;
            }
            ...
            $inner_join = ' INNER JOIN `'._DB_PREFIX_.'deotemplate_profiles_lang` pl ON (pl.id_deotemplate_profiles = p.id_deotemplate_profiles) AND id_lang='.(int)$id_lang;
            $inner_join .= ' INNER JOIN `'._DB_PREFIX_.'deotemplate_profiles_shop` ps ON (ps.id_deotemplate_profiles = p.id_deotemplate_profiles)';
            $sql = 'SELECT p.id_deotemplate_profiles, p.name, p.profile_key, pl.id_lang, ps.id_shop, ps.active from `'._DB_PREFIX_.'deotemplate_profiles` p '.$inner_join.$where;

            $active_profile = Db::getInstance()->getRow($sql);
            
            if ($active_profile) {
                return $active_profile['id_deotemplate_profiles'];
            }
    ...
    public static function getAllProfileRewrite($id_profile = null)
    {
        if ($id_profile) {
            ...
            $inner_join = ' INNER JOIN `'._DB_PREFIX_.'deotemplate_profiles_lang` pl ON (pl.id_deotemplate_profiles = p.id_deotemplate_profiles)';
            $inner_join .= ' INNER JOIN `'._DB_PREFIX_.'deotemplate_profiles_shop` ps ON (ps.id_deotemplate_profiles = p.id_deotemplate_profiles)';
            $sql = 'SELECT p.id_deotemplate_profiles, p.name, pl.id_lang, pl.friendly_url from `'._DB_PREFIX_.'deotemplate_profiles` p '.$inner_join.$where;
