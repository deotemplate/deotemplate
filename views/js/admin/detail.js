...
$(document).ready(function() {
    ...
    $('.element-list .plist-element').draggable({
        ...
        revert: "true",
        helper: "clone",
        appendTo: '.admin-pagebuilder-detail',
        scroll: false,
        zIndex: 10000,
        // handle: ".waction-drag",
    ...
    $('#deotemplate_details_form').submit(function(e) {
        // e.preventDefault();
        genreateForm();
    });
    ...
    function genreateForm(){
        ...
        ObjectFrom.objectForm = returnObjElemnt('.admin-pagebuilder-detail .hook-content');
    ...
    function saveData(){
        $(".btn-savewidget").click(function(){
            ...
            if ($('.active-formmodal').data('element') == 'product_cover_thumbnails'){
                $('.active-formmodal').find('.size').html(data['size']);
            }

            if ($('.active-formmodal').data('element') == 'product_cover_thumbnails' && data['type'] == 'thumbnail'){
                $('.active-formmodal').find('.thumb').removeClass('hide-thumb').html(deo_thumbnail_position[data['thumb']]);
            }else{
                $('.active-formmodal').find('.thumb').addClass('hide-thumb');
            }

            if ($('.active-formmodal').data('element') == 'product_more_info'){
                $('.active-formmodal').find('.type').html(deo_product_more_info[data['type']]);
            }
    ...
    function setProFormAction(){
        ...
        $(document).on('change', '.type', function() {
            if ($(this).val() == 'thumbnail'){
                $('.config-image.show-all').hide(400);
                $('.config-image.with-thumb').show(400);
                $('.thumb-position').trigger('change');
                $('.responsive').trigger('change');

            }else{
                $('.config-image.with-thumb').hide(400);
                $('.config-image.show-all').show(400);
                $('.image-size').show(400);
            }
        });

        $(document).on('change', '.responsive', function() {
            if ($('.thumb-position').val() == 'none' || $('.responsive').val() == '0'){
                $(this).closest('.formmodal').find('.responsive-thumb').hide(400);

                return true;
            }

            if ($(this).val() == 1){
                $(this).closest('.formmodal').find('.responsive-thumb').show(400);
            }else{ 
                $(this).closest('.formmodal').find('.responsive-thumb').hide(400);
            }
        });
        ...
        $(document).on('change', '.zoom', function() {
            if ($(this).val() == 'none' || $(this).val() == 'in' || $(this).val() == 'out_scrooll'){
                $(this).closest('.formmodal').find('.select-zoom-none').hide(400);
            }else{
                $(this).closest('.formmodal').find('.select-zoom-none').show(400);
            }
        });

        $(document).on('change', '.thumb-position', function() {
            if ($(this).val() == 'none'){
                $(this).closest('.formmodal').find('.image-size').hide(400);
                $(this).closest('.formmodal').find('.thumb-position-none').hide(400);
                $(this).closest('.formmodal').find('.responsive-thumb').hide(400);
            }else{
                $(this).closest('.formmodal').find('.image-size').show(400);
                $(this).closest('.formmodal').find('.thumb-position-none').show(400);
                if ($('.responsive') == 1){
                    $(this).closest('.formmodal').find('.responsive-thumb').show(400);
                }
            }
        ...
        $(document).on('click', '#home_wrapper .element-config', function (e) {
            ...
            $('#modal_form .type').trigger('change');
            $('#modal_form .zoom').trigger('change');
            if ($('#modal_form .type').val() == 'thumbnail'){
                $('#modal_form .responsive').trigger('change');
                $('#modal_form .thumb-position').trigger('change');
            }
        });
