	...
	<div class="deo-more-product-img pro" data-idproduct="{$product.id}">
		...
		<div class="views_block slick-row">
			<div class="list-thumbs slick-slider deo-carousel deo-carousel-loading"
				...
				data-fade="{$fade}" 
				data-slideactive="{if $product.cover}active{/if}"
				data-col_loading="{$col_loading}" 
				data-deo_size="{$deo_size}"
			>
				{foreach from=$product.images item=image name=thumbnails}
					<div id="thumbnail_{$image.id_image|intval}" class="thumbnail-image slick-slide {$col_loading}">
						<a href="{$image.bySize.large_default.url}" data-idproduct="{$product.id_product|intval}" rel="other-views" class="link-img image-hover thickbox-ajax-{$product.id_product|intval} thickbox-ajax{if $image.id_image == $product.default_image.id_image} shown{/if}" title="{if !empty($image.legend)}{$image.legend}{else}{$product.name}{/if}">
							{if $deo_lazyload}
