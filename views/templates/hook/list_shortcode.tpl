...
<html>
	...
	<body>
		...
		<div class="list-sc-header">
			<a target="_blank" class="sc-direct btn btn-info" href="{$shortcode_url}">{l s='List Shortcode' mod='deotemplate'}</a>
			<a target="_blank" class="sc-direct btn btn-success" href="{$shortcode_url_add}">{l s='Create Shortcode' mod='deotemplate'}</a>
		</div>
			...
			<div class="list-sc-content">
				{foreach from=$list_shortcode item=list_shortcode_item}
					<div class="list-sc-item{if !$list_shortcode_item['active']} inactive{/if}" data-shortcode-key="{$list_shortcode_item['shortcode_key']}">
						...
						<div class="sc-status">
							(ID: {$list_shortcode_item['id_deotemplate_shortcode']} - 
							{if $list_shortcode_item['active']}
								{l s='Active' mod='deotemplate'}					
							{else}
								{l s='Inactive' mod='deotemplate'}
							{/if}
							)
						</div>
					</div>
		...
		<script type="text/javascript">
			$(document).ready(function(){
				$('.list-sc-item').click(function(){
					...
					var shortcode_txt = '[DeoSC sc_key='+shortcode_key+'][/DeoSC]';
