...
<div class="widget-row code plist-element {if isset($defaultItem)}active{else}{($eItem.form.active && isset($eItem.form.active)) ? 'active' : 'deactive'}{/if}" data-element='code' data-form="{$eItem.dataForm|escape:'html':'UTF-8'}">
    ...
    <div class="content-code sort-content">
        <textarea name="filecontent" rows="5" class="">{if isset($eItem.code) && !isset($defaultItem)}{$eItem.code nofilter}{/if}</textarea>
    </div>
</div>